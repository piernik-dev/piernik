...
FLAT_PROBLEM_NAME=${PROBLEM_NAME//\//___}
...
GOLD_DIR=${OUT_DIR}${FLAT_PROBLEM_NAME}_gold_dir
OBJ_PREFIX=obj_
...
rm -rf $GOLD_DIR ${OBJ_PREFIX}$GOLD_OBJ ${OBJ_PREFIX}$TEST_OBJ ${RUNS_DIR}/${FLAT_PROBLEM_NAME}_$TEST_OBJ $GOLD_LOG ${OUT_DIR}${OBJ_PREFIX}${TEST_OBJ}

for i in $GOLD_OBJ $TEST_OBJ ; do
    d=${RUNS_DIR}/${FLAT_PROBLEM_NAME}_$i
    [ ! -d $d ] && mkdir $d
done
...
cp ${GOLD_DIR}/runs/$(basename ${PROBLEM_NAME})_${GOLD_OBJ}/problem.par ${RUNS_DIR}/${FLAT_PROBLEM_NAME}_${GOLD_OBJ}/
cp runs/$(basename ${PROBLEM_NAME})_${TEST_OBJ}/problem.par ${RUNS_DIR}/${FLAT_PROBLEM_NAME}_${TEST_OBJ}/
...
(
    ...
    make -j ${OBJ_PREFIX}{$GOLD_OBJ,$TEST_OBJ} > ${FLAT_PROBLEM_NAME}.make_stdout 2>&1
)

for i in $GOLD_OBJ $TEST_OBJ ; do
    rm ${RUNS_DIR}/${FLAT_PROBLEM_NAME}_${i}/${PIERNIK} 2> /dev/null || echo "${PROBLEM_NAME}: rundir clean"
    cp ${TMP_DIR}/${OBJ_PREFIX}${i}/${PIERNIK} ${RUNS_DIR}/${FLAT_PROBLEM_NAME}_$i
done

# detect whether we have genuine riemann setup (which is unlikely to run in RTVD)
grep -qi riemann ${RUNS_DIR}/${FLAT_PROBLEM_NAME}_${GOLD_OBJ}/problem.par && RIEMANN=1 || RIEMANN=0

cd ${RUNS_DIR}/${FLAT_PROBLEM_NAME}_${TEST_OBJ}
rm *.h5 2> /dev/null

if [ $RIEMANN == 1 ] ; then
    eval $RUN_COMMAND ./${PIERNIK} > ${FLAT_PROBLEM_NAME}.test_Riemann_stdout
else
    eval $RUN_COMMAND ./${PIERNIK} "-n '&NUMERICAL_SETUP solver_str = \"RTVD\" /'"  > ${FLAT_PROBLEM_NAME}.test_RTVD_stdout &
    RIEM=Riemann
    (
	...
	cp ../problem.par .
	(
	    eval $RUN_COMMAND ./${PIERNIK} "-n '&NUMERICAL_SETUP solver_str = \"Riemann\" /'"  > ${FLAT_PROBLEM_NAME}.test_Riemann_stdout 2> $RIEM_ERR
...
cd ${RUNS_DIR}/${FLAT_PROBLEM_NAME}_$GOLD_OBJ
...
if [ ! -e ${OUTPUT} ] ; then
    eval $RUN_COMMAND ./${PIERNIK} $GOLD_PARAMS > ${FLAT_PROBLEM_NAME}.gold_stdout &
...
if [ $RIEMANN == 0 ] ; then
    ...
    if [ -e ${RUNS_DIR}/${FLAT_PROBLEM_NAME}_${TEST_OBJ}/${RIEM}/${OUTPUT} ] ; then
	./bin/gdf_distance ${RUNS_DIR}/${FLAT_PROBLEM_NAME}_${TEST_OBJ}{,/${RIEM}}/${OUTPUT} 2>&1 | tee $RIEM_LOG
