         ...
         if (p /= proc) then
            ...
            if (this%ms%sl(p, IN)%total_size /= 0) then ! we have something to communicate with process p
               if (tgt3d) then
                  ...
                  do i = lbound(this%ms%sl(p, OUT)%list, dim=1), this%ms%sl(p, OUT)%cur_last !ubound(this%ms%sl(p, OUT)%list, dim=1)
      ...
      do p = FIRST, LAST
         if (p /= proc) then
            if (this%ms%sl(p, IN)%total_size /= 0) then ! we have something received from process p
               if (tgt3d) then
                  do i = lbound(this%ms%sl(p, IN)%list, dim=1), this%ms%sl(p, IN)%cur_last !ubound(this%ms%sl(p, IN)%list, dim=1)
