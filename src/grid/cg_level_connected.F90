         ...
         if (allocated(cg%pi_tgt%seg)) then
            do g = lbound(seg(:), dim=1), ubound(seg(:), dim=1)
               if (d4) then
                  ...
                  call MPI_Irecv(seg(g)%buf4, size(seg(g)%buf4, kind=4), MPI_DOUBLE_PRECISION, seg(g)%proc, seg(g)%tag, p1v_comm, req%nxt(), err_mpi)
               else
                  call MPI_Irecv(seg(g)%buf,  size(seg(g)%buf,  kind=4), MPI_DOUBLE_PRECISION, seg(g)%proc, seg(g)%tag, p1v_comm, req%nxt(), err_mpi)
               endif
            enddo
         endif
         end associate
      ...
      do while (associated(cgl))
         ...
         do g = lbound(seg(:), dim=1), ubound(seg(:), dim=1)
            ...
            if (d4) then
               ...
               seg(g)%buf4(:, :, :, :) = p4d
               call MPI_Isend(seg(g)%buf4, size(seg(g)%buf4, kind=4), MPI_DOUBLE_PRECISION, seg(g)%proc, seg(g)%tag, p1v_comm, req%nxt(), err_mpi)
            else
               p3d => cg%q(iv)%span(cse)
               seg(g)%buf(:, :, :) = p3d
               call MPI_Isend(seg(g)%buf,  size(seg(g)%buf,  kind=4), MPI_DOUBLE_PRECISION, seg(g)%proc, seg(g)%tag, p1v_comm, req%nxt(), err_mpi)
            endif
         enddo
         end associate
         ...
         if (allocated(cgl%cg%pib_tgt%seg)) then
            do g = lbound(seg(:), dim=1), ubound(seg(:), dim=1)
               if (present(arr4d)) then
                  ...
                  call MPI_Irecv(seg(g)%buf4, size(seg(g)%buf4, kind=4), MPI_DOUBLE_PRECISION, seg(g)%proc, seg(g)%tag, pbfc_comm, req%nxt(), err_mpi)
               else
                  call MPI_Irecv(seg(g)%buf,  size(seg(g)%buf,  kind=4), MPI_DOUBLE_PRECISION, seg(g)%proc, seg(g)%tag, pbfc_comm, req%nxt(), err_mpi)
               endif
            enddo
         endif
         end associate
         ...
         if (allocated(cgl%cg%pob_tgt%seg)) then
            do g = lbound(seg(:), dim=1), ubound(seg(:), dim=1)
               ...
               if (present(arr4d)) then
                  ...
                  call MPI_Isend(seg(g)%buf4, size(seg(g)%buf4, kind=4), MPI_DOUBLE_PRECISION, seg(g)%proc, seg(g)%tag, pbfc_comm, req%nxt(), err_mpi)
               else
                  seg(g)%buf(:, :, :)     = cgl%cg%q(ind)%arr(   cse(xdim, LO):cse(xdim, HI), cse(ydim, LO):cse(ydim, HI), cse(zdim, LO):cse(zdim, HI))
                  call MPI_Isend(seg(g)%buf,  size(seg(g)%buf,  kind=4), MPI_DOUBLE_PRECISION, seg(g)%proc, seg(g)%tag, pbfc_comm, req%nxt(), err_mpi)
               endif
            enddo
         endif
         end associate
            ...
            do g = lbound(cg%ri_tgt%seg(:), dim=1), ubound(cg%ri_tgt%seg(:), dim=1)
               associate (seg => cg%ri_tgt%seg(g))
                  if (d4) then
                     ...
                     call MPI_Irecv(seg%buf4, size(seg%buf4, kind=4), MPI_DOUBLE_PRECISION, seg%proc, seg%tag, r1v_comm, req%nxt(), err_mpi)
                  else
                     call MPI_Irecv(seg%buf,  size(seg%buf,  kind=4), MPI_DOUBLE_PRECISION, seg%proc, seg%tag, r1v_comm, req%nxt(), err_mpi)
                  endif
               end associate
            enddo
         endif
      ...
      do while (associated(cgl))
         ...
         do g = lbound(cg%ro_tgt%seg(:), dim=1), ubound(cg%ro_tgt%seg(:), dim=1)
            ...
            associate (seg => cg%ro_tgt%seg(g))
               ...
               if (d4) then
                  call MPI_Isend(seg%buf4, size(seg%buf4, kind=4), MPI_DOUBLE_PRECISION, seg%proc, seg%tag, r1v_comm, req%nxt(), err_mpi)
               else
                  call MPI_Isend(seg%buf,  size(seg%buf,  kind=4), MPI_DOUBLE_PRECISION, seg%proc, seg%tag, r1v_comm, req%nxt(), err_mpi)
               endif
            end associate
         enddo
