...
!<
   ...
   subroutine balance_fill_lowest(this)
      ...
      use mpisetup,        only: piernik_MPI_Allreduce, master, FIRST, LAST, comm, err_mpi, nproc
      use sort_piece_list, only: grid_piece_list
...
!> \brief collect local proposed patches on a given level into an array on the master process
   ...
   subroutine patches_to_list(this, gp, ls)
      ...
      if (slave) call MPI_Isend(ls, I_ONE, MPI_INTEGER, FIRST, tag_ls, comm, req(nreq), err_mpi)
      ...
      if (master) then !> \warning Antiparallel
         ...
         deallocate(gptemp)
         do p = FIRST + 1, LAST
            call MPI_Recv(rls, I_ONE, MPI_INTEGER, p, tag_ls, comm, MPI_STATUS_IGNORE, err_mpi)
            if (rls > 0) then
   ...
   subroutine distribute_patches(this, gp, from)
      ...
      if (master) then
         ...
         do p = FIRST + I_ONE, LAST
            ...
            call MPI_Send(ls, I_ONE, MPI_INTEGER, p, tag_lsR, comm, err_mpi)
            if (ls>0) then
               ...
               enddo
               call MPI_Send(gptemp, size(gptemp, kind=4), MPI_INTEGER8, p, tag_gptR, comm, err_mpi)
               deallocate(gptemp)
            endif
         enddo
      else
         ! receive new, perhaps more balanced patches
         call MPI_Recv(ls, I_ONE, MPI_INTEGER, FIRST, tag_lsR, comm, MPI_STATUS_IGNORE, err_mpi)
         if (ls>0) then
            allocate(gptemp(I_OFF:I_END,ls))
            call MPI_Recv(gptemp, size(gptemp, kind=4), MPI_INTEGER8, FIRST, tag_gptR, comm, MPI_STATUS_IGNORE, err_mpi)
            do s = lbound(gptemp, dim=2, kind=4), ubound(gptemp, dim=2, kind=4)
