...
!> \brief Initialization - continued after allocation of everything interesting
   ...
   subroutine init_multigrid_grav
      ...
      if (.not. allocated(coarsest%level%gse)) call die("[multigrid_gravity:init_multigrid_grav] cannot determine number of pieces on coaarsest level")
      cnt = 0
      do p = FIRST, LAST
         if (allocated(coarsest%level%gse)) cnt = cnt + size(coarsest%level%gse(p)%c(:))
      enddo
