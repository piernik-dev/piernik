...
! OPT: try an explicit loop over n to see if better pipelining can be achieved
   ...
   subroutine relaxing_tvd(n, u, u0, bb, divv, cs_iso2, istep, sweep, i1, i2, dx, dt, cg)
      ...
      use constants,        only: one, zero, half, GEO_XYZ, LEFT, RIGHT, INV_CENTER
...
!locals
      ...
      if (full_dim) then
         ...
         if (dom%geometry_type == GEO_XYZ) then
            ...
            u1(:,2:n) = u0(:,2:n) - rk2coef(integration_order,istep) * gc(INV_CENTER,:,2:n) * dtx * ( gc(RIGHT,:,2:n)*fu(:,2:n) - gc(LEFT,:,2:n)*fu(:,1:n-1) )
         endif
         u1(:,1)   = u1(:,2)
      else
