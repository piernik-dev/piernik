...
#ifdef CRESP
                  tot_q(T_CREN) = tot_q(T_CREN) + drvol * sum(sum(pu(iarr_crspc_n,   ii, :, :), dim=1), mask=cg%leafmap(i, :, :))
                  tot_q(T_CREE) = tot_q(T_CREE) + drvol * sum(sum(pu(iarr_crspc_e,   ii, :, :), dim=1), mask=cg%leafmap(i, :, :))
                  tot_q(T_ENCR) = tot_q(T_ENCR) + tot_q(T_CREE)
#endif /* CRESP */
                  tot_q(T_ENER) = tot_q(T_ENER) + tot_q(T_ENCR)
#endif /* COSM_RAYS */
