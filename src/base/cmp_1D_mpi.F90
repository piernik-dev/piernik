...
!<
   ...
   subroutine compare_real_array1D(arr)
      ...
      use mpisetup,   only: slave, LAST, comm, proc, err_mpi
      ...
      if (proc /= LAST) call MPI_Send(arr(:), size(arr(:), kind=4), MPI_DOUBLE_PRECISION, proc+I_ONE, tag, comm, err_mpi)
      if (slave) then
         call MPI_Recv(aux(:), size(aux(:), kind=4), MPI_DOUBLE_PRECISION, proc-I_ONE, tag, comm, MPI_STATUS_IGNORE, err_mpi)
...
!> \brief Compare a 1D integer array across all processes. Die if any difference is found
   ...
   subroutine compare_int_array1D(arr)
      ...
      if (proc /= LAST) call MPI_Send(arr(:), size(arr(:), kind=4), MPI_INTEGER, proc+I_ONE, tag, comm, err_mpi)
      if (slave) then
         call MPI_Recv(aux(:), size(aux(:), kind=4), MPI_INTEGER, proc-I_ONE, tag, comm, MPI_STATUS_IGNORE, err_mpi)
...
!> \brief Compare a 1D integer array across all processes. Die if any difference is found
   ...
   subroutine compare_string(str)
      ...
      if (proc /= LAST) call MPI_Send(str, len(str, kind=4), MPI_CHARACTER, proc+I_ONE, tag, comm, err_mpi)
      if (slave) then
         call MPI_Recv(aux, len(aux, kind=4), MPI_CHARACTER, proc-I_ONE, tag, comm, MPI_STATUS_IGNORE, err_mpi)
