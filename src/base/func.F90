...
contains
   ...
   subroutine dipol(xi,yj,zk,phi,theta,amp,rmk)
     ...
     real, dimension(:,:,:,:), allocatable :: A, temp
#ifndef ISO
     ...
     if(.not.allocated(A).and. .not.allocated(temp)) then
        allocate(A(3,nx,ny,nz))
        allocate(temp(2,nx,ny,nz))
     else
     ...
     do i = 1,nx
       ...
       do j = 1,ny
         ...
         do k = 1,nz 
           ...
           temp(1,i,j,k) = -1.0 *Aphi* y / rc
           temp(2,i,j,k) = Aphi * x / rc

           A(1,i,j,k) =  temp(1,i,j,k)*cos(theta)*cos(phi) - temp(2,i,j,k)*sin(phi)
           A(2,i,j,k) =  temp(1,i,j,k)*cos(theta)*sin(phi) + temp(2,i,j,k)*cos(phi)
           A(3,i,j,k) = -temp(1,i,j,k)*sin(theta)
         enddo
       enddo
     enddo
     ...
     if (allocated(temp)) deallocate(A)
