...
!----------------------------------------------------------------------------------------------------
   ...
   subroutine cr_species_products_spallation(i,j,k,u_cell,dt_doubled)
      ...
      dgas = 0.0
      if (has_ion) dgas = dgas + u_cell(flind%ion%idn, i, j, k) / mp
      if (has_neu) dgas = dgas + u_cell(flind%neu%idn, i, j, k) / mH
...
!                      dcr_n = min(u_cell(iarr_crspc2_n(cr_prim,:)), dcr_n)
      ...
      !print *, 'U_cell Shape : ',shape(u_cell(iarr_crspc2_n(:,:), i, j, k))
      !print *, 'U_src Shape : ',shape(usrc_cell(iarr_crspc2_n(:,:), i, j, k))

      !stop
      do i_spc = 1, nspc

         u_cell(iarr_crspc2_n(i_spc,:), i, j, k) = u_cell(iarr_crspc2_n(i_spc,:), i, j, k) + dt_doubled*usrc_cell(iarr_crspc2_n(i_spc,:))
         u_cell(iarr_crspc2_e(i_spc,:), i, j, k) = u_cell(iarr_crspc2_e(i_spc,:), i, j, k) + dt_doubled*usrc_cell(iarr_crspc2_e(i_spc,:))
