...
!<
   ...
   subroutine ionized_index(flind)
      ...
      flind%ion%get_cs => ionized_sound_speed

   end subroutine ionized_index

!>
!! \brief (MH/JD) [R] Sound speed computation for the ionized fluid
!!
...
!<
   ...
   pure function ionized_sound_speed(cg, fl, i, j, k) result(cs)
...
#ifdef MAGNETIC
      bx = (cg%b(xdim,i,j,k) + cg%b(xdim, i+dom%D_x, j,         k        ))/(I_ONE + dom%D_x)
      by = (cg%b(ydim,i,j,k) + cg%b(ydim, i,         j+dom%D_y, k        ))/(I_ONE + dom%D_y)
      bz = (cg%b(zdim,i,j,k) + cg%b(zdim, i,         j,         k+dom%D_z))/(I_ONE + dom%D_z)
...
#endif /* !ISO */
   end function ionized_sound_speed
