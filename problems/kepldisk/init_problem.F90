...
! Restrictions: using only cor bnd_cond, disk radius must be smaller than xmax and ymax
  ...
  namelist /PROBLEM_CONTROL/  problem_name, run_id, &
                              rhoa,d0,r_max,mtr, mf_orient, pres_cor, collf
...
!-----------------------------------------------------------------------------
  ...
  subroutine init_prob
    ...
    use constants, only :   cm,kpc,mp,r_gc_sun,sek,gram,year,au
    use grid, only      :   dx
    use hydrostatic, only : hydrostatic_zeq
    use start, only     :   xmin,nb,smalld,c_si,alpha,gamma,collfaq,dimensions
#ifndef ISO
...
! rowniez po restarcie 
    ...
    do i = 1,nx
      ...
      d0=(dcmol+dcneut+dcion+dchot) * 1.36 * mp*500000.0
      if(dimensions .eq. '3d') call hydrostatic_zeq(i, 0, d0, dprof)
...
!    stop
	...
	do k=1,nz
	 ...
	 rs = sqrt(xi**2+yj**2+zk**2)
	   ...
	   u(idna,i,j,k) = rhoa + d0/(r_max*0.001)/cosh(min((rc/r_max)**mtr,100.0))
	 endif
	 u(idna,i,j,k) = u(idna,i,j,k)/100.0
	 ...
	 case('toroidal')
      ...
      enddo
    enddo
    write(*,*) 'collfaq = ',collfaq/cm**3*sek*gram
    write(*,*) ' '
