...
def draw_particles(ax, p1, p2, pm, nbins, min1, max1, min2, max2, drawd):
    if nbins > 1:
        ax.hist2d(p1, p2, nbins, weights=pm, range=[[min1, max1], [min2, max2]], norm=matplotlib.colors.LogNorm())
        if not drawd:
            ax.set_facecolor('xkcd:black')
    else:
        ax.scatter(p1, p2, marker=".")
...
def plotcompose(pthfilen, var, output, options):
    umin, umax, cmap, sctype, cu, cx, cy, cz, drawd, drawp, nbins, uaxes = options
    ...
    if drawp:
        ax = draw_particles(ax, px, pz, pm, nbins, xmin, xmax, zmin, zmax, drawd)
    ...
    if drawp:
        ax = draw_particles(ax, py, px, pm, nbins, ymin, ymax, xmin, xmax, drawd)
    ...
    if drawp:
        ax = draw_particles(ax, py, pz, pm, nbins, ymin, ymax, zmin, zmax, drawd)
    ax.set_xlabel("y [%s]" % pu.labelx()(ulen))
    ax.set_ylabel("z [%s]" % pu.labelx()(ulen))
