...
COMMON data, workdir, tsl_file, namearr, darray, nvar,npoint,last_dir
COMMON state, nhy, time, firstfile, firstnhy
...
;    namearr = STRSPLIT(line, ' ',/extract,count=nvar)
    namearr = STRSPLIT(line, ' ',/extract)
    nvar = N_ELEMENTS(namearr)         ; --- change 
...
;message, /reset_error_state   ; --- not working in GDL
